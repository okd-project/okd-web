# Volunteer Opportunities 

<!--- cSpell:ignore virt -->

The OKD Working Group welcomes all contributions. Here are some specific areas where we could use some volunteers. 

| Item | Description | Contact |
| --- | ----------- | --- |
|  OKDerators | Help in the building of operators for the OKD Operator Catalog  | Owen Howard (Slack preferred) |
|  UPI (libvirt) | Testing UPI installs/upgrades of OKD versions, testing functionality includes:<br/>- LDAP auth<br/>- Trident Operator<br/>- Worker machineset scaling<br/>- Monitoring stack with persistence  | Tyrone Wilson (Slack preferred) |

