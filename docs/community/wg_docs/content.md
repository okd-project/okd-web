...
Images are embedded in a page using the standard Markdown syntax `![description](URL)`, but the image can be formatted with [Attribute Lists](https://python-markdown.github.io/extensions/attr_list/)<!--{: target="_blank" .external } comment for docusaurus compat-->.  This is most commonly used to scale an image or center an image, e.g.
...
The [Admonition](https://python-markdown.github.io/extensions/admonition/)<!--{: target="_blank" .external } comment for docusaurus compat--> extension allows you to add themed information boxes using the `!!!` and `???` syntax:
...
There are some additional features available due to the highlight plugin installed in MkDocs.  Full details can be found in the [MkDocs Materials documentation](https://squidfunk.github.io/mkdocs-material/reference/code-blocks/)<!--{target=_blank} comment for docusaurus compat-->.
...
This project uses [cSpell](https://github.com/streetsidesoftware/cspell)<!--{target=_blank} comment for docusaurus compat--> to check spelling within the markdown.  The configuration included in the project automatically excludes content in a code block, enclosed in triple back quotes \`\`\`.
